...
def scanForStigmatizingLanguage():
    ...
    for chunk in model.stream(finalPrompt):
        ...
        for i in x:
            if i.replace("(", "").replace(")", "").replace("\"","") not in allWords:
                ...
                clinical_note_display.after(0, highlight_text, allWords)
            index += 1
    ...
    modified_text_content = og_text_content
    modified_highlights = allWords
...
frameBottomLeft = tk.Frame(root)
frameBottomLeft.grid(row=2, column=0)

frameBottomRight = tk.Frame(root)
frameBottomRight.grid(row=2, column=1)
